...
public class PaymentController {
    ...
    public CommonResult create(@RequestBody Payment payment){
        ...
        log.info("**********插入结果:"+res);
        if(res>0){
            return new CommonResult<Integer>(200,"插入数据成功,serverPort:"+port,res);
        }else {
    ...
    public CommonResult getPayentById(@PathVariable("id") Long id){
    ...
    public String getPaymentLib(){
        return port;
    }

    @GetMapping(value="/payment/getPort")
    public String getPayentById(){
        return port;
    }
